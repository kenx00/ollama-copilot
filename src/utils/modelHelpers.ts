...
export async function initializeOllamaClient(): Promise<boolean> {
  try {
    ...
    console.log("[modelHelpers.ts] Read apiHost:", apiHost);
    // Log before connecting
    console.log("[modelHelpers.ts] Trying to connect to Ollama at:", apiHost);
...
export async function getAvailableOllamaModels(): Promise<ModelInfo[]> {
  ...
  while (retries < maxRetries) {
    try {
      ...
      console.log(
        `Successfully retrieved ${response.models.length} models from Ollama`
      );
      ...
      lastError = error;
      console.error(
        `Error getting Ollama models (attempt ${retries + 1}):`,
        error
      );
      retries++;
