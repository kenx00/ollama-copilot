...
export class OllamaApiService extends Disposable implements IOllamaApiService {
  ...
  constructor(
    ...
    super();
    this.instanceId = ++OllamaApiService.instanceCounter;
    console.log(
      `[OllamaApiService.ts] Constructor called. Instance ID: ${this.instanceId}`
    );
    // Initialize API host from configuration
    this._apiHost = this.configService.get<string>(
      "apiHost",
    ...
    this.track(
      this.configService.onDidChangeConfiguration((event) => {
        console.log(
          `[OllamaApiService.ts] [${this.instanceId}] onDidChangeConfiguration callback fired`
        );
        const affectsOllamaApiHost =
          event.affectsConfiguration("ollama.apiHost");
        console.log(
          `[OllamaApiService.ts] [${this.instanceId}] affectsConfiguration('ollama.apiHost'):`,
          affectsOllamaApiHost
        );
        if (affectsOllamaApiHost) {
          setTimeout(() => {
            const newHost = this.configService.get<string>(
              "apiHost",
              "http://localhost:11434"
            );
            console.log(
              `[OllamaApiService.ts] [${this.instanceId}] (deferred) Detected apiHost change, updating to:`,
              newHost
            );
            this._apiHost = newHost;
            this.ollamaClient = new Ollama({ host: this._apiHost });
            console.log(
              `[OllamaApiService.ts] [${this.instanceId}] (deferred) ollamaClient updated with host:`,
              this._apiHost
            );
          }, 0);
        }
  ...
  setApiHost(host: string): void {
    ...
    this.configService.update("apiHost", host).catch((error) => {
  ...
  async listModels(): Promise<ModelInfo[]> {
    console.log(
      `[OllamaApiService.ts] [${this.instanceId}] listModels using host:`,
      this._apiHost
    );
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes("ECONNREFUSED")) {
        throw new Error(
          "Failed to connect to Ollama. Please ensure the Ollama service is running on " +
            this._apiHost
        );
      }
      throw new Error(
  ...
  async generate(options: GenerateOptions): Promise<string> {
    console.log(
      `[OllamaApiService.ts] [${this.instanceId}] generate using host:`,
      this._apiHost
    );
    try {
      ...
      if (
        (!system && options.prompt.includes("code completion")) ||
        options.prompt.includes("Complete the")
      ) {
        system =
          "You are a code completion assistant. Always return ONLY raw code without any markdown formatting, code fences (```), or language identifiers. Never wrap code in markdown blocks.";
      }
  ...
  async generateStream(
    ...
    console.log(
      `[OllamaApiService.ts] [${this.instanceId}] generateStream using host:`,
      this._apiHost
    );
    try {
      let fullResponse = "";
  ...
  async chat(
    ...
    console.log(
      `[OllamaApiService.ts] [${this.instanceId}] chat using host:`,
      this._apiHost
    );
    Logger.debug("OllamaApiService", "chat called", {
      model,
  ...
  async chatStream(
    ...
    console.log(
      `[OllamaApiService.ts] [${this.instanceId}] chatStream using host:`,
      this._apiHost
    );
    try {
      ...
      return {
        ...
        created_at:
          lastResponse.created_at instanceof Date
  ...
  async healthCheck(): Promise<boolean> {
    Logger.info(
      "OllamaApiService",
      `Performing health check on: ${this._apiHost}`
    );
    try {
      ...
      Logger.info(
        "OllamaApiService",
        `Health check successful, found ${models.length} models`
      );
      return true;
    } catch (error) {
